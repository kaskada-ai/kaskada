...
Imagine a world where LLMs are constantly watching the world around and looking out for your interests.
Instead of using GPT to draft an e-mail, ask the agent to send it for you.
As e-mails arrive, they are constantly triaged, with some receiving immediate (generated) responses, most being provided in occassionally summaries for your attention, and only the most important actually interrupting you.
As news articles are posted, they may be filtered and grouped based on your interests and summarized when you request.
[BeepGPT](https://epinzur.github.io/posts/001-openai-fine-tuning-1/post.html), an example agent that automatically listens to messages on Slack and pings you if appropriate is just the beginning.
Such agents are "always on" and tapped into the firehose of evetns happening around you.
...
An email asking about tour availability demands a prompt answer, while a grocery list from a relative might not need any action.
...
1. Watching what happens in real-time, so the agent can respond (if appropriate) and respond with awareness of what has happened.
2. Knowing when to act by triggering actions based on the events.
