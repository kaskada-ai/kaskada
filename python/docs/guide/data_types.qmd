...
listing:
  - id: types
    template: data_types/_table.ejs
    contents:
      - data_types.yml
---

Kaskada operates on typed Timestreams.
Similar to how every Pandas `DataFrame` has an associated `dtype`, every Kaskada `Timestream` has an associated [PyArrow data type](https://arrow.apache.org/docs/python/api/datatypes.html) returned by [`kaskada.Timestream.data_type`](../reference/timestream_class.qmd#kaskada.Timestream.data_type).
