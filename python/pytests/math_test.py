...
SourceCols = NamedTuple('SourceCols', [('m', kd.Timestream), ('n', kd.Timestream), ('datatype', pa.DataType)])
...
@pytest.fixture(scope="module", params=[pa.int8(), pa.uint8(), pa.int64(), pa.uint64(), pa.float64()], ids=datatype_id)
...
async def test_add(source, golden) -> None:
    golden.jsonl(source.m + source.n)

async def test_add_literal(source, golden) -> None:
    golden.jsonl(source.m + 1)
