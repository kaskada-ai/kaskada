...
async def test_add(source, golden) -> None:
    golden.jsonl(kd.record({"m": source.m, "n": source.n, "result": source.m + source.n}))
...
async def test_powf(source, golden) -> None:
    golden.jsonl(kd.record({"m": source.m, "n": source.n, "result": source.m.powf(source.n)}))
...
async def test_clamp_min(source, golden) -> None:
    golden.jsonl(kd.record({"m": source.m, "n": source.n, "result": source.m.clamp(min=source.n)}))

async def test_clamp_max(source, golden) -> None:
    golden.jsonl(kd.record({"m": source.m, "n": source.n, "result": source.m.clamp(max=source.n)}))

async def test_clamp_min_literal_max(source, golden) -> None:
        golden.jsonl(kd.record({"m": source.m, "n": source.n, "result": source.m.clamp(min=source.n, max = 8)}))

async def test_clamp_max_literal_min(source, golden) -> None:
        golden.jsonl(kd.record({"m": source.m, "n": source.n, "result": source.m.clamp(min=0, max=source.n)}))
