...
def test_float_seconds(golden) -> None:
    ...
    table = kd.sources.Pandas(df, time_column_name="time", key_column_name="user", time_unit="s")
...
def test_float_milliseconds(golden) -> None:
    ...
    df = pd.DataFrame(data)
    table = kd.sources.Pandas(df, time_column_name="time", key_column_name="user", time_unit="ms")

    golden.jsonl(table)

def test_float_nanoseconds(golden) -> None:
    data = {
        'time': [1671477472026119000],
        'user': ['tom']
    }
    df = pd.DataFrame(data)
