...
async def test_float_seconds(golden) -> None:
    ...
    table = await kd.sources.Pandas.create(df, time_column="time", key_column="user", time_unit="s")
...
async def test_float_milliseconds(golden) -> None:
    ...
    df = pd.DataFrame(data)
    table = await kd.sources.Pandas.create(df, time_column="time", key_column="user", time_unit="ms")
...
async def test_float_nanoseconds(golden) -> None:
    ...
    df = pd.DataFrame(data)
    table = await kd.sources.Pandas.create(df, time_column="time", key_column="user", time_unit="ns")
