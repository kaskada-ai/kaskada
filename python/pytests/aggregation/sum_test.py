...
def source_spread_across_days() -> kd.sources.PyList:
    ...
    ], time_column_name="time", key_column_name="key")
...
def test_sum_since_hourly(source_spread_across_days, golden) -> None:
    golden.jsonl(source_spread_across_days.col("m").sum(window=kd.windows.Since(kd.Timestream.hourly)))