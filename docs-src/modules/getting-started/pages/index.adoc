...
When you make a query, the resulting timeline is output in one of two ways: as a _history_ or as a _snapshot_. 

* A timeline *History* contains a value each time the timeline changes, and each row is associated with a different entity and point in time. 
* A timeline *Snapshot* contains a value for each entity at the _same_ point in time; each row is associated with a different entity, but all rows are associated with the same time.
