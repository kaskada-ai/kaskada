...
* The first player likes to brag about his victories
* The game was too hard for the second player
* The third player pays for upgrades when she gets frustrated.
...
----
{ loss_dur:<1> sum(GameVictory.duration)<2> }
----
...
{ loss_dur: sum(GameVictory.duration) } <1>
   <2> | when(count(GameDefeat, window=since(GameVictory)) == 2) <3>
----
...
{ loss_dur: sum(GameVictory.duration) }
   ...
   <2> | shift_to(time_of($input) | add_time(hours(1))) <3>
----
