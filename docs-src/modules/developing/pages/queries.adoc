...
By default, query results are written to a Parquet file: the location of these files are included along with additional query metadata in the query response.
...
To see the result files, you can use a JSON parser such as https://stedolan.github.io/jq/[jq] to inspect the `outputTo.objectStore.outputPaths.paths` array:
