...
/// Evaluator for the `lte_primitive` (less than or equal) instruction.
struct LtePrimitiveEvaluator<T: ArrowNumericType> {
    lhs: PrimitiveValue<T>,
    rhs: PrimitiveValue<T>,
}

impl<T: ArrowNumericType> Evaluator for LtePrimitiveEvaluator<T> {
...
fn create<T: ArrowNumericType>(
    ...
    Ok(Box::new(LtePrimitiveEvaluator::<T> {
        lhs: lhs.primitive()?,
        rhs: rhs.primitive()?,
    }))
}
