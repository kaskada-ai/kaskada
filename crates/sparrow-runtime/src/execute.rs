...
pub async fn execute_long_lived(
    request: ExecuteRequest,
    s3_helper: S3Helper,
    _flight_record_local_path: Option<std::path::PathBuf>,
    _flight_record_header: FlightRecordHeader,
    ...
    if request.final_result_time.is_some() {
        error_stack::bail!(Error::Internal(
            "long lived materialization should not have final-at-time"
        ))
    }
    ...
    // and store new state. Create a new storage path for the local store to
    // exist.
    if request.compute_snapshot_config.is_some() {
        error_stack::bail!(Error::Internal("snapshots not supported"))
    };
    ...
    tracing::debug!("Beginning long-lived materialization process");
    if request.limits.is_some() {
        error_stack::bail!(Error::Internal(
            "limits not supported for long-lived process"
        ))
    }
    let runtime_options = RuntimeOptions {
    ...
    Ok(compute_executor.execute_with_progress(s3_helper, None, None))
}
