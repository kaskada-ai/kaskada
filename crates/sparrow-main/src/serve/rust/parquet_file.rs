...
use parquet::arrow::{parquet_to_arrow_schema_by_columns, ParquetRecordBatchStreamBuilder};
...
impl ParquetFile {
    ...
    pub async fn read_stream(
        ...
        BoxStream<'static, error_stack::Result<RecordBatch, fenl_catalog::scan::Error>>,
        Error,
    > {
        let path = self.path.clone();
        let batch_stream = ParquetRecordBatchStreamBuilder::new(self)
            .await
            .into_report()
        ...
        Ok(batch_stream
            .map(move |batch| {
                batch
                    .into_report()
                    .change_context(fenl_catalog::scan::Error::FailedToScanSource)
                    .attach_printable_lazy(|| path.clone())
            })
            .boxed())
    }
