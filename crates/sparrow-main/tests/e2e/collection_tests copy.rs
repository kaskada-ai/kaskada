...
use arrow::{
    array::{
        Float64Builder, GenericStringBuilder, Int64Builder, MapBuilder, StringBuilder,
        UInt64Builder,
    },
...
async fn test_u64_to_str_get_static_key() {
    ...
    insta::assert_snapshot!(QueryFixture::new("{ f1: get(4 as u64, Input.u64_to_s) }").run_to_csv(&arrow_collection_data_fixture().await).await.unwrap(), @r###"
    _time,_subsort,_key_hash,_key,f1
