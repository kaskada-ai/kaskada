...
use super::{binary_merge, BinaryMergeResult};
...
impl HeterogeneousMerge {
    ...
    pub fn new(result_type: &DataType, datatype_l: &DataType, datatype_r: &DataType) -> Self {
        Self {
            ...
            spread_l: Spread::try_new(false, datatype_l).expect("spread"),
            spread_r: Spread::try_new(false, datatype_r).expect("spread"),
    ...
    pub fn merge(&mut self) -> error_stack::Result<Batch, Error> {
        // 1. Binary merge
        // 2. Get spread bits
        // 3. Get merged key columns
        // 4. Update grouping
        // 5. Do appropriate spread (? does spread::signaled always work ?)
        // 6. This returns an Arrayref, which should be your merged `_data`
        // 7. Place that in new Batch. Make sure watermark doesn't change
        // 8. That's your output.
        todo!()
    }
