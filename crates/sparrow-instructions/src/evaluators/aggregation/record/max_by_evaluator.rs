...
use crate::{
    AggFn, AggregationArgs, Evaluator, EvaluatorFactory, MaxByAccumToken, RuntimeInfo, StateToken,
    StaticInfo,
};
...
pub struct MaxByEvaluator<AggF, T>
where
    AggF: AggFn,
    AggF::AccT: Serialize + DeserializeOwned,
    T: Serialize + DeserializeOwned,
{
    token: MaxByAccumToken<AggF::AccT, T>,
    args: AggregationArgs<ValueRef>,
}

impl<AggF, T> Evaluator for MaxByEvaluator<AggF, T>
where
    AggF: AggFn,
...
impl<AggF, T> EvaluatorFactory for MaxByEvaluator<AggF, T>
where
    AggF: AggFn + Send + 'static,
    AggF::AccT: Serialize + DeserializeOwned + Sync + Default,
    T: Serialize + DeserializeOwned + Sync + Send + Default,
{
    fn try_new(info: StaticInfo<'_>) -> anyhow::Result<Box<dyn Evaluator>> {
        let args = AggregationArgs::from_input(info.args)?;
        match args {
            AggregationArgs::NoWindow { .. } | AggregationArgs::Since { .. } => {
                let token = MaxByAccumToken::<AggF::AccT, T>::default();
...
impl<AggF, T> MaxByEvaluator<AggF, T>
where
    AggF: AggFn,
    AggF::AccT: Serialize + DeserializeOwned,
    T: Serialize + DeserializeOwned + Clone,
{
    fn ensure_entity_capacity(token: &mut MaxByAccumToken<AggF::AccT, T>, entity_id_len: usize) {
...
mod tests {
    ...
    fn test_sum_f64() {
        ...
        let output =
            MaxByEvaluator::<Sum<Float64Type>>::aggregate(&mut accum, 3, &entity_indices, &input)
                .unwrap();
    ...
    fn test_sum_i64_nulls() {
        ...
        let output =
            MaxByEvaluator::<Sum<Int64Type>>::aggregate(&mut accum, 3, &entity_indices, &input)
                .unwrap();
    ...
    fn test_max_f64() {
        ...
        let output =
            MaxByEvaluator::<Max<Float64Type>>::aggregate(&mut accum, 3, &entity_indices, &input)
                .unwrap();
    ...
    fn test_max_i64() {
        ...
        let output =
            MaxByEvaluator::<Max<Int64Type>>::aggregate(&mut accum, 3, &entity_indices, &input)
                .unwrap();
        let output = downcast_primitive_array::<Int64Type>(output.as_ref()).unwrap();
    ...
    fn test_mean_f64() {
        ...
        let output =
            MaxByEvaluator::<Mean<Float64Type>>::aggregate(&mut accum, 3, &entity_indices, &input)
                .unwrap();
    ...
    fn test_mean_i64() {
        ...
        let output =
            MaxByEvaluator::<Mean<Int64Type>>::aggregate(&mut accum, 3, &entity_indices, &input)
                .unwrap();
    ...
    fn test_last_f64() {
        ...
        let output = MaxByEvaluator::<LastPrimitive<Float64Type>>::aggregate(
            &mut accum,
            3,
            &entity_indices,
            &input,
    ...
    fn test_first_f64() {
        ...
        let output = MaxByEvaluator::<FirstPrimitive<Float64Type>>::aggregate(
            &mut accum,
            3,
            &entity_indices,
            &input,
