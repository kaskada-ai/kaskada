...
use crate::{
    ...
    TopStringAccumToken, TwoStacksTopStringAccumToken,
};

use super::two_stacks_top_string_evaluator::TwoStacksTopStringEvaluator;

/// Evaluator for the `Top` instruction on strings.
pub struct TopStringEvaluator {
    args: AggregationArgs<ValueRef>,
    token: TopStringAccumToken,
}

impl Evaluator for TopStringEvaluator {
...
impl EvaluatorFactory for TopStringEvaluator {
    fn try_new(info: StaticInfo<'_>) -> anyhow::Result<Box<dyn Evaluator>> {
        let args = AggregationArgs::from_input(info.args)?;
        match args {
            AggregationArgs::NoWindow { .. } | AggregationArgs::Since { .. } => {
                let token = TopStringAccumToken::default();
                Ok(Box::new(Self { token, args }))
            }
            AggregationArgs::Sliding { .. } => {
                let token = TwoStacksTopStringAccumToken::new();
                Ok(Box::new(TwoStacksTopStringEvaluator { token, args }))
            }
        }
    }
}

impl TopStringEvaluator {
    fn ensure_entity_capacity(token: &mut TopStringAccumToken, entity_id_len: usize) {
...
mod tests {
    ...
    fn test_string_first_with_multiple_chunks() {
        ...
        let mut token = TopStringAccumToken::default();
        let output = TopStringEvaluator::aggregate(&mut token, 1, &entity_indices, &input).unwrap();
    ...
    fn test_string_first_with_no_null() {
        ...
        let mut token = TopStringAccumToken::default();
        let output = TopStringEvaluator::aggregate(&mut token, 3, &entity_indices, &input).unwrap();
    ...
    fn test_string_first_with_null() {
        ...
        let output = TopStringEvaluator::aggregate(&mut token, 4, &entity_indices, &input).unwrap();
