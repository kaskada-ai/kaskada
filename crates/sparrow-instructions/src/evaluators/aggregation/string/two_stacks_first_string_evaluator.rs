...
use crate::{
    AggregationArgs, Evaluator, FirstString, RuntimeInfo, TwoStacks,
    TwoStacksStringAccumToken,
};

/// Evaluator for the `First` instruction on strings.
pub(crate) struct TwoStacksFirstStringEvaluator {
    pub args: AggregationArgs<ValueRef>,
    pub token: TwoStacksStringAccumToken<FirstString>,
}

impl Evaluator for TwoStacksFirstStringEvaluator {
...
impl TwoStacksFirstStringEvaluator {
    fn ensure_entity_capacity(
        token: &mut TwoStacksStringAccumToken<FirstString>,
...
mod tests {
    ...
    #[test]
    fn test_sliding_string_first_with_no_null() {
        ...
        let output = TwoStacksFirstStringEvaluator::aggregate(
            &mut token,
            1,
            &entity_indices,
            &input,
