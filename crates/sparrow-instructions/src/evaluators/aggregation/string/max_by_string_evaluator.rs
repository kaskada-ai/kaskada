...
use crate::{
    AggregationArgs, Evaluator, EvaluatorFactory, MaxByStringAccumToken, RuntimeInfo, StateToken,
    StaticInfo,
};

/// Evaluator for the `MaxBy` instruction on strings.
pub struct MaxByStringEvaluator {
    args: AggregationArgs<ValueRef>,
    token: MaxByStringAccumToken,
}

impl Evaluator for MaxByStringEvaluator {
...
impl EvaluatorFactory for MaxByStringEvaluator {
    fn try_new(info: StaticInfo<'_>) -> anyhow::Result<Box<dyn Evaluator>> {
        let args = AggregationArgs::from_input(info.args)?;
        match args {
            AggregationArgs::NoWindow { .. } | AggregationArgs::Since { .. } => {
                let token = MaxByStringAccumToken::default();
                Ok(Box::new(Self { token, args }))
            }
            AggregationArgs::Sliding { .. } => {
                // let token = TwoStacksMaxByStringAccumToken::new();
                // Ok(Box::new(TwoStacksMaxByStringEvaluator { token, args }))
                todo!()
            }
        }
    }
...
impl MaxByStringEvaluator {
    fn ensure_entity_capacity(token: &mut MaxByStringAccumToken, entity_id_len: usize) {
    ...
    #[inline]
    fn update_accum(
        token: &mut MaxByStringAccumToken,
        entity_index: u32,
        input_is_valid: bool,
        input: &str,
    ) -> anyhow::Result<Option<String>> {
        if input_is_valid {
            token.put_value(entity_index, Some(input.to_owned()))?;
        }
    ...
    fn update_since_accum(
        ...
        if reset_window {
            token.reset_value_for_entity(entity_index)?;
        };
...
mod tests {
    ...
    fn test_string_first_with_multiple_chunks() {
        ...
        let mut token = MaxByStringAccumToken::default();
        let output =
            MaxByStringEvaluator::aggregate(&mut token, 1, &entity_indices, &input).unwrap();
    ...
    fn test_string_first_with_no_null() {
        let entity_indices = UInt32Array::from(vec![0, 1, 1, 1, 1, 0]);
        ...
        let mut token = MaxByStringAccumToken::default();
        let output =
            MaxByStringEvaluator::aggregate(&mut token, 3, &entity_indices, &input).unwrap();
    ...
    fn test_string_first_with_null() {
        ...
        let output =
            MaxByStringEvaluator::aggregate(&mut token, 4, &entity_indices, &input).unwrap();
