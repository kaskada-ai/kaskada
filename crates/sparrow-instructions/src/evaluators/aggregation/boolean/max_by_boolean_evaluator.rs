...
use super::two_stacks_first_boolean_evaluator::TwoStacksMaxByBooleanEvaluator;
use crate::{
    AggregationArgs, BooleanAccumToken, Evaluator, EvaluatorFactory, RuntimeInfo, StateToken,
    StaticInfo, TwoStacksBooleanAccumToken,
};

/// Evaluator for the `MaxBy` instruction on booleans.
pub struct MaxByBooleanEvaluator {
    args: AggregationArgs<ValueRef>,
    token: BooleanAccumToken,
}

impl Evaluator for MaxByBooleanEvaluator {
...
impl EvaluatorFactory for MaxByBooleanEvaluator {
    fn try_new(info: StaticInfo<'_>) -> anyhow::Result<Box<dyn Evaluator>> {
        ...
        match args {
            ...
            AggregationArgs::Sliding { .. } => {
                ...
                Ok(Box::new(TwoStacksMaxByBooleanEvaluator { token, args }))
            }
        }
    }
}

impl MaxByBooleanEvaluator {
...
mod tests {
    ...
    fn test_boolean_first_with_no_null() {
        ...
        let output =
            MaxByBooleanEvaluator::aggregate(&mut token, 3, &entity_indices, &input).unwrap();
        assert_eq!(
    ...
    fn test_boolean_first_with_null() {
        ...
        let output =
            MaxByBooleanEvaluator::aggregate(&mut token, 4, &entity_indices, &input).unwrap();
        assert_eq!(
            downcast_boolean_array(output.as_ref()).unwrap(),
            &BooleanArray::from(vec![
                Some(false),
