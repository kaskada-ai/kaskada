...
/// Evaluator for `get` on maps for boolean keys and boolean values.
#[derive(Debug)]
pub(in crate::evaluators) struct GetBooleanToBooleanEvaluator {
    map: ValueRef,
    key: ValueRef,
}

impl EvaluatorFactory for GetBooleanToBooleanEvaluator {
    fn try_new(info: StaticInfo<'_>) -> anyhow::Result<Box<dyn Evaluator>> {
        ...
        anyhow::ensure!(
            ...
            "expected boolean value type, saw {:?}",
            value_type
        );
...
impl Evaluator for GetBooleanToBooleanEvaluator {
