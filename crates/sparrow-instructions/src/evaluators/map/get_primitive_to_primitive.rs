...
#[derive(Debug)]
pub(in crate::evaluators) struct GetPrimitiveToPrimitiveEvaluator<T, U>
where
    T: ArrowPrimitiveType + Sync + Send,
    U: ArrowPrimitiveType + Sync + Send,
{
    map: ValueRef,
    key: ValueRef,
...
impl<T, U> EvaluatorFactory for GetPrimitiveToPrimitiveEvaluator<T, U>
where
    T: ArrowPrimitiveType + Sync + Send,
    U: ArrowPrimitiveType + Sync + Send,
{
...
impl<T, U> Evaluator for GetPrimitiveToPrimitiveEvaluator<T, U>
where
    T: ArrowPrimitiveType + Sync + Send,
    U: ArrowPrimitiveType + Sync + Send,
{
    fn evaluate(&mut self, info: &dyn crate::RuntimeInfo) -> anyhow::Result<ArrayRef> {
        ...
        let result: PrimitiveArray<U> = {
            anyhow::ensure!(
                key_input.len() == map_input.len(),
                "key and map lengths don't match"
            );
            let values: Vec<Option<U::Native>> = (0..key_input.len())
