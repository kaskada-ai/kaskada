...
use arrow::array::{ArrayRef, AsArray, ListBuilder, StringBuilder, StructBuilder};
...
impl EvaluatorFactory for CollectStructEvaluator {
    fn try_new(info: StaticInfo<'_>) -> anyhow::Result<Box<dyn Evaluator>> {
        ...
        Ok(Box::new(Self {
            ...
            tick,
            duration,
            token: CollectStructToken::default(),
        }))
    }
}
...
impl CollectStructEvaluator {
    ...
    fn evaluate_non_windowed(&mut self, info: &dyn RuntimeInfo) -> anyhow::Result<ArrayRef> {
        ...
        // You're getting an input of a struct array
        // You have an existing List<StructArray> (per entity)
        // You need to add to that list and produce it per input (add it to some output listarray)
        //
        //

        let input = input.as_struct();
        let struct_builder = StructBuilder::new(fields, field_builders);
        let builder = ListBuilder::new(struct_builder);
